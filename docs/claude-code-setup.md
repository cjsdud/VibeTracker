...
"검증 필요" 기능이 쌓였을 때 설정 화면의 검증 세션 프롬프트를 실행하면 Claude Code가
목록을 받아 기능마다 (1) 연결 테스트 실행 또는 실제 동작 확인 → (2) 통과/실패를
`record_work_update`로 정직하게 기록한다.

- **동작을 직접 관찰한 것만 통과로 기록** — 타입체크/빌드 성공은 검증이 아니다
- **검증과 수리를 분리** — 실패를 발견해도 고치지 않고 보고만 한다 (안심하고 반복 실행 가능)
- 기능 하나만 검증하려면 기능 지도에서 그 기능을 눌러 "검증 요청 프롬프트 복사" 사용

## 7. 복귀 브리핑 자동 주입 (SessionStart 훅, 권장)
