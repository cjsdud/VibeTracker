...
## 0. 웹 클로드 코드(claude.ai)에서 쓰는 경우 — 가장 쉬운 방법

웹 세션은 매번 GitHub에서 저장소를 새로 받아오므로, 연결 설정이 전부 **저장소에
커밋**되어 있어야 한다. 설정 화면의 **"웹 클로드 코드에서 쓰는 경우" 원샷 프롬프트**를
웹 세션에 붙여넣으면 Claude가 알아서 처리한다:

1. `.mcp.json` 생성 (MCP 서버 주소 + 토큰)
2. `CLAUDE.md`에 작업 규칙 + projectId 추가
3. 복귀 브리핑 SessionStart 훅 설치 (`.claude/` 아래)
4. 전부 커밋·푸시

완료 후 **새 세션**을 열고 "기능 지도를 만들어서 등록해줘"라고 말하면 된다.

- 복귀 브리핑 훅은 웹 컨테이너에서도 동작한다 (프록시 경유 환경을 위해 fetch 실패 시
  curl로 자동 폴백)
- 환경의 네트워크 액세스가 VibeTrack 서버 도메인에 허용되어 있어야 한다
- 토큰이 저장소에 평문 커밋되므로 **비공개 저장소에서만** 사용하고, 유출 의심 시
  설정 화면에서 폐기·재발급한다
...
- 위의 모든 작업(품질 경고 수정, 히스토리 연결)이 끝나기 전에는 승인을 요청하지 마라.
  전부 끝난 뒤에만 나에게 "등록을 마쳤으니 VibeTrack 웹에서 기능 지도를 검토하고
  승인해 주세요"라고 한 번만 알려라.
```
